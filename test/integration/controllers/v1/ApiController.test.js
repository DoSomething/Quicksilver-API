...
(function() {
  ...
  describe('Requests to the root (/api) path', function() {
    ...
    it('GET: Redirects to v1 path and returns v1 route details', function(done) {
      request(sails.hooks.http.app).get('/api').expect('content-type', /json/).end(function(err, res) {
        if (err) {
          throw err;
        }
  ...
  describe('Requests to v1 root (/api/v1) path', function() {
    ...
    it('GET: Returns v1 paths', function(done) {
      request(sails.hooks.http.app).get('/api/v1').expect('content-type', /json/).end(function(err, res) {
        if (err) {
          throw err;
        }
